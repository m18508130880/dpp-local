...
public class TcpClient extends Thread
{
	...
	private AlertCtrl m_AlertCtrl;
	
	public TcpClient(DBUtil dbUtil, AlertCtrl alertCtrl) throws Exception
	{
		m_DBUtil = dbUtil;
			...
			try
			{
				...
				if(dTime > m_TimeOut)
				{
					...
					else
					{
						...
						SetSendMsgList(msg);
						CommUtil.PRINT("Send Active Package to DPP-AppSvr!");	
					}
					testTime = nowTime;
				}			
			}
			...
			while(true)
			{
				try
				{
					...
					if(nRcvLen <= 0)
					{
						objSocket.close();
						objSocket = null;
						System.out.println("DPP Server Closed the socket");
						break;
					}
