...
public class TaskListBean extends RmiBean 
{
	...
	public void ajaxGetToday(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone)
	{
		try 
		{
			getHtmlData(request);
	...
	public List<Object> objToJson(ArrayList<?> list, List<Object> CData){
		...
		while (iterator.hasNext()) {
			...
			System.out.println("SN["+RealJson.SN+"]");
			System.out.println("Start_Time["+RealJson.Start_Time+"]");
			System.out.println("End_Time["+RealJson.End_Time+"]");
			System.out.println("Finish_Time["+RealJson.Finish_Time+"]");
			System.out.println("Type["+RealJson.Type+"]");
			System.out.println("Uploader["+RealJson.Uploader+"]");
			System.out.println("Status["+RealJson.Status+"]");
			System.out.println("GJ_List["+RealJson.GJ_List+"]");
			System.out.println("GX_List["+RealJson.GX_List+"]");
			System.out.println("Project_Id["+RealJson.Project_Id+"]");
			System.out.println("Des["+RealJson.Des+"]");
			System.out.println("Finish_Des["+RealJson.Finish_Des+"]");
			CData.add(json);
		}
		return CData;
	...
	public String getSql(int pCmd)
	{
		...
		{
			case 0://查询全部
				Sql = " select t.sn,  t.start_time, t.end_time, t.finish_time, t.type, t.uploader, t.status, t.gj_list, t.gx_list, t.project_id, t.des, t.finish_des "
						+ " from task_list t order by t.end_time";
				break;
			case 1://查询项目全部
				Sql = " select t.sn,  t.start_time, t.end_time, t.finish_time, t.type, t.uploader, t.status, t.gj_list, t.gx_list, t.project_id, t.des, t.finish_des "
						+ " from task_list t "
						+ "where project_id = '" + Project_Id + "' order by t.end_time";
				break;
			case 2://查询项目全部，时间区间
				Sql = " select t.sn,  t.start_time, t.end_time, t.finish_time, t.type, t.uploader, t.status, t.gj_list, t.gx_list, t.project_id, t.des, t.finish_des "
						+ " from task_list t "
						+ " where project_id = '" + Project_Id + "'"
						+ " and t.start_time >= date_format('"+currStatus.getVecDate().get(0).toString()+"', '%Y-%m-%d %H-%i-%S')"
						+ " and t.end_time <= date_format('"+currStatus.getVecDate().get(1).toString()+"', '%Y-%m-%d %H-%i-%S')"
			...
			case 4://查询项目全部，小于某个时间
				...
				break;
			case 10://添加
				Sql = " insert into task_list(start_time, end_time, finish_time, type, uploader, status, gj_list, gx_list, project_id, des, finish_des) "
						+ " values('"+Start_Time+"', '"+End_Time+"', '"+Finish_Time+"', '"+Type+"', '"+Uploader+"', '"+Status+"', '"+GJ_List+"', '"+GX_List+"', '"+Project_Id+"', '"+Des+"', '"+Finish_Des+"')";
				break;
		}
		return Sql;
	}
