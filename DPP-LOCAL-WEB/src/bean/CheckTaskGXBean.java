...
public class CheckTaskGXBean extends RmiBean
{
	...
	public void getCheckGX(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone) throws ServletException, IOException
	{
		getHtmlData(request);
		currStatus = (CurrStatus) request.getSession().getAttribute("CurrStatus_" + Sid);
		currStatus.getHtmlData(request, pFromZone);
		...
		if (msgBean.getStatus() == MsgBean.STA_SUCCESS)
		{
			...
			{
				CheckTaskGXBean bean = (CheckTaskGXBean)iterator.next();
				Id = bean.getId();
				Start_Id = bean.getStart_Id();
				End_Id = bean.getEnd_Id();
				Road = bean.getRoad();
