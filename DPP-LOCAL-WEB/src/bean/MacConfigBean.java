...
public class MacConfigBean extends RmiBean 
{
	...
	public String getSql(int pCmd)
	{
		...
		{
			case 0://查询
				Sql = " select t.id, t.cname, t.demo, t.pwd, t.onoff, t.ctime"
						+ " from device_detail t order by t.id";
				break;
			case 10://添加
				Sql = " insert into device_detail(id, cname, demo, pwd, ctime)" +
					  " values('"+ PId +"', '"+ TId +"', '"+ CName +"', '"+ PWD +"')";
				break;
			case 11://编辑
				Sql = " update device_detail t set t.id= '"+ PId +"', t.cname= '"+ TId +"', t.demo= '"+ CName +"', t.pwd= '"+ PWD +"', t.ctime= '" + CTime + "'" +
					  " where t.id = '" + PId +"'";
				break;
		}
		return Sql;
