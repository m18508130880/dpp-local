...
public class WeatherBean extends RmiBean
{
	...
	public void getWeatherNow(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone) throws ServletException, IOException
	{
		getHtmlData(request);
		currStatus = (CurrStatus) request.getSession().getAttribute("CurrStatus_" + Sid);
		currStatus.getHtmlData(request, pFromZone);
		...
		if (msgBean.getStatus() == MsgBean.STA_SUCCESS)
		{
			...
			String json = JSONObject.toJSONString(weather);
			Resp = "0000" + json;
		}
		request.getSession().setAttribute("CurrStatus_" + Sid, currStatus);
		outprint.write(Resp);
